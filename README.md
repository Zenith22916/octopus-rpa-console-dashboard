...
- **机器人状态**（运维）：每台机器人实时在跑什么、排队/空闲多久、近 24 小时与近 7 天负载
- **日志检索**（运维）：跨机器人 + 跨运行记录搜日志关键词，可只筛异常/警告，命中行直达详情页对应日志
- **触发器合规**（运维）：排期时刻 vs 实际运行比对，输出按时/延迟/应用不符/漏跑，含各机器人命中率
...
web/（纯前端，hash 路由八视图）
    ...
    # 版面：左侧固定导航栏（品牌 + 视图组/运维组 + 数据时间）+ 右侧主内容区
...
| 元素 | 说明 |
|---|---|
| 导航菜单 | 八个视图切换，分「视图 / 运维」两组；按钮与列表之间有透明桥接层，鼠标下移不会闪收 |
...
> 侧栏分两组：**视图**（运行数据分析 / 任务时间轴 / 触发器排期 / 项目控制台）与
> **运维**（机器人状态 / 日志检索 / 触发器合规）。运行记录详情由链接进入，不在导航里。
...
- 顶部指标与每台机器人一张卡片，按「运行中 → 排队中 → 空闲，近 7 天记录数降序」排列
- **状态口径**（由运行记录推断，见 `dashboard.build_bot_status`）：有 `end` 为空且 `Executing` 的记录 = 运行中；无运行中但有在途记录 = 排队中；没有任何在途记录 = 空闲
- 卡片内容：当前任务名 + 已跑时长 / 排队时长（点击直达该条记录日志）、空闲时长 + 最近一次应用、近 24 小时次数与失败数、近 7 天次数/失败率（≥20% 标红）/平均排队/平均运行
- 筛选：全部 / 仅忙（运行中 + 排队中）/ 仅空闲；随「自动更新」每 60 秒刷新（每台机器人的状态由内存运行记录推算）

### 日志检索（`#/logsearch`，运维）

- 跨机器人、跨运行记录按关键词检索日志正文，筛选：关键词 / 机器人 / 时间范围（1、3、7 天）/ 级别（全部行、异常+警告、仅错误）
- 命中行以关键词居中截取片段（超长行首尾加省略号）+ 关键词高亮，附机器人、应用、`文件:行号` 与错误/警告标签
- **点击命中行直达该条运行记录的日志**（跳 `#/detail?id=…&kw=…&file=…`，详情页加载完后自动切到对应文件、填入关键词并定位到第一处命中）
