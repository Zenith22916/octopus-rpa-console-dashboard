...
- **运行记录详情**：单条记录完整信息、局域网共享日志目录检索、Monaco 只读查看器（VSCode 同款高亮/minimap）、异常高亮与搜索、选择机器人后重新运行
- **项目控制台**：云端项目列表 + 项目详情 + 选择机器人运行 + 飞书配置中心在线编辑
...
| POST | `/api/projects/run` | 触发某应用运行（`bot_id` 可选，不传则自动复用历史成功机器人） |
| POST | `/api/rerun` | 详情页「重新运行」（按 flow_id 触发，`bot_id` 可选） |
...
- **直接拖动平移**：在图表上按住拖动即可平移时间轴，1:1 跟手（整幅宽度 = 整个窗口，拖 100px 就走 100px 的时间），没有滚动条；向左拖看更新、向右拖看更早，拖到「现在」或最早数据处自动停住
- 滚轮：向下 = 向未来（最多到"现在"），向上 = 向过去；双击图表：恢复实时跟随
- 触屏可直接拖动平移（横向手势交给平移，纵向仍可滚页面）；拖动后那一下不会误触发「点块进详情」
- 所有状态的数据块（含运行中）点击均可跳转详情查看日志

### 运行记录详情（`#/detail?id=`）

- 左侧基础信息卡：机器人、状态、触发方式、触发器、流程 ID（flow_id）、流程编号（process_no）、起止时间、开始运行、排队/运行时长
- 顶部按钮：**↻ 重新运行**（**先弹窗选择执行机器人**，再调用 `/api/rerun`，返回批次号与执行机器人）、**⚙ 项目配置**（跳转 `#/projects?fid=` 并选中该项目）、**← 返回上一页**（无历史兜底回时间轴）
  - 弹窗数据来自 `/api/bots?flow_id=`，分三组：**本流程运行过的机器人**（默认选中最近一次成功运行的那台，带「推荐」标）、**在线可用的其他机器人**、**离线 / 已停用机器人**（默认收起）；另有「不指定，由平台分配」项
...
- 红色虚线 = 当前时刻与今天（10 秒刷新）
- **导出 Excel**：按当前筛选（视图 + 机器人 + 状态）生成 xlsx 并触发下载，三张表（首表为日历）——
  - `排期日历`：版式对齐页面图表。行 = 时刻 × 应用，列 = 周几（月视图为当月有排期的日期）；同一应用在该时刻的排期日**合并成一个色块**（跨机器人合并，颜色取图表调色板，灰色 = 已停用），「时刻」列在同一时刻的多行之间纵向合并，空白格保留边框
  - `排期明细`：与图表同口径展开的排期点（周视图按周几、月视图按日期），含触发器名/触发方式/周期类型/状态/更新时间
  - `触发器汇总`：命中筛选的全部触发器（含 Webhook 与未排期项），比图表多一列「排期描述」（如「每周周一 08:00」）
  - 说明：网页是取首尾列画矩形，会跨过中间没排期的日子；日历表只在**日期连续**时才并成一块，避免色块横跨没排期的日期
