...
- **运行分析仪表盘**：概览指标 + 触发/状态分布 + 星期×小时热力图 + 各机器人排队与负载 + 失败看板
- **运行记录时间轴**：每台机器人的真实运行记录（手动 / 定时 / Webhook），排队与执行两段彩色区分
- **运行记录详情**：单条记录的完整信息、局域网共享日志目录检索、VSCode 风格缩略图导航、异常高亮与搜索
- **触发器日程表**：触发器每周/每月循环任务时刻表

## 效果图

### 运行分析仪表盘（默认主页）

![运行分析仪表盘](assets/effect_analysis.png)

### 运行记录时间轴

![运行记录时间轴](assets/effect_timeline.png)
...
![触发器日程表](assets/effect_schedule.png)
...
web/（纯前端，hash 路由四视图）：
    ...
    #/analysis（默认主页） #/timeline #/detail?id= #/schedule
```
