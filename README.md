...
| 指标卡 | **所有视图的指标卡统一放标题栏**（`#headMetrics`），按视图整组切换显隐（位置与 `HEAD_METRIC_BOXES` 一一对应）：时间轴=Webhook/手动/定时记录数+机器人；分析=总运行/运行中/失败等；排期=触发器总数/已启用/已停用/Webhook；项目=项目总数/已配映射/未配映射/配置组数；机器人状态=机器人/运行中/排队中/空闲/在途任务；日志检索=命中行/命中日志文件/候选记录/扫描行数；合规=应跑次数/按时/应用不符/漏跑/命中率 |
...
- **手机端（≤880px）自动转置**：时间改沿竖直方向（过去在上、现在在下），机器人横向排开，块内文字**逐字竖排**（字正立、每字一行，放不下按能排几个字截断）；横向窄屏因此能把长边让给时间轴。左侧时间刻度拆成两行「日期 / 时刻」（`09-22` 换行 `11:00`），免得日期被裁掉。断点切换（横竖屏翻转）会自动重画
- **数据拆分**：每条记录按「排队（蓝）→ 执行（状态色）」两段区分，整条用白色边框框起；块内文字单行居中、过长截断加 `...`，少于 8 字不显示
- **时间窗口**：下拉档位 30 分钟 ~ 7 天（默认 2 小时；窄高窗口自动改为 6 小时，横向更舒展）；Ctrl+滚轮可自由缩放到 10 分钟 ~ 30 天，下拉会自动留空表示「自定义窗口」
- **触发方式筛选**：全部 / 手动 / 定时触发器 / Webhook
- **直接拖动平移**：在图表上按住拖动即可平移时间轴，1:1 跟手（整幅长度 = 整个窗口，桌面按宽度、手机转置后按高度换算）；内容跟着手走——桌面横向拖（左=更晚、右=更早），手机转置后竖向拖（上=更晚、下=更早），拖到「现在」或最早数据处自动停住
- **底部窗口指示条**：滑块宽度 = 当前窗口占全部数据的比例（窗口越长滑块越宽），拖滑块或点轨道空白可平移；它只是指示器，窗口长度不能靠拖它改
- **缩放窗口**：`Ctrl/⌘ + 滚轮` 以鼠标所指时刻为锚点缩放（向上放大、向下缩小，范围 10 分钟 ~ 30 天）；也可用上方「窗口」下拉，或双击恢复默认
- 滚轮：向下 = 向未来（最多到"现在"），向上 = 向过去；双击图表：恢复实时跟随
- 触屏可直接拖动平移；桌面「横向手势归平移、纵向仍可滚页面」，手机转置后图表区域由平移独占手势（要滚页面请在图表外滑动）；拖动后那一下不会误触发「点块进详情」
...
- **标题栏指标卡**：项目总数 / 已配映射 / 未配映射 / 配置组数（映射 = 该项目是否已关联配置组，加载失败或列表为空时自动清空）
...
- **两张图**（表格只有几行，用它们把竖向空间填起来，数据都由内存运行记录现算，见 `bsChartsRender`）：
  - 右上「**近 7 天运行量**」：每台机器人一根堆叠横条（绿=成功、红=失败），右端标合计；悬停看成功/失败/合计明细
  - 下方「**近 24 小时负载（机器人 × 小时）**」：行=机器人、列=整点小时，格子颜色越暖表示该小时跑得越多（色阶与分析页热力图一致）
  - 手机端两张图改为纵向堆叠、各给固定高度（图表容器在手机上不靠 flex 分配高度）
...
- **标题栏指标卡**：应跑次数 / 按时 / 应用不符 / 漏跑 / 命中率（原先在页面顶部，现已上移到标题栏与其他视图统一）；左侧「各机器人命中率」总览，右侧排期明细（可点行进详情看日志）
...
| 指标卡 | 一行横排 | 一行**横向滑动**（卡宽由内容定、不折行，`overflow-x:auto` + 隐藏滚动条；划到两端不带动整页） |
