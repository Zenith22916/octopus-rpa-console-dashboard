...
- **手机端（≤880px）自动转置**：时间改沿竖直方向（过去在上、现在在下），机器人横向排开，块内文字**逐字竖排**（字正立、每字一行，放不下按能排几个字截断）；横向窄屏因此能把长边让给时间轴。断点切换（横竖屏翻转）会自动重画
...
- **直接拖动平移**：在图表上按住拖动即可平移时间轴，1:1 跟手（整幅长度 = 整个窗口，拖 100px 就走 100px 的时间）；桌面横向拖（左=更新、右=更早），手机转置后竖向拖（下=更新、上=更早），拖到「现在」或最早数据处自动停住
